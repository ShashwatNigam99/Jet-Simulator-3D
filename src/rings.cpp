...
///////////////////////////////////////////
SmokeRing::SmokeRing(float x, float y,float z, float rot, color_t color) {
    this->position = glm::vec3(x, y, z);
    this->rotation = rot;
    this->theta = rand()% 360;
    ...
    this->object = create3DObject(GL_TRIANGLES, (iter/3)*p, vertex_buffer_data, color, GL_LINE);
}
...
void SmokeRing::set_position(float x, float y) {
    this->position = glm::vec3(x, y, 0);
}

void SmokeRing::tick() {
  // this->rotation += 0.1;
  this->position.x+= 0.1 * cos(this->theta * M_PI/180.0f);  //  std::cout<<"ring :"<<this->position.x<<","<<this->position.y<<"\n";
  this->position.y+= 0.1 * sin(this->theta * M_PI/180.0f);  //  std::cout<<"ring :"<<this->position.x<<","<<this->position.y<<"\n";
}
